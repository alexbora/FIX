...
#define __STDC_WANT_LIBEXT1__ value
#endif

#define API (const uint8_t *)"c537779d863d054eb803522a53fac09c"
#define SECRET \
	(const uint8_t*)"Th/ydPigXK7kdROh31J9tgBoCsU6kgOc3WDINrGBotzcRaVlWekuNFGK1sVnS3GVG2Jp1wTHASLjIPUcVclkbw=="
...
account *account_new(logon *login) {
	...
	*out = (account){.dec = malloc(BUF), .key = NULL};
	...
	out->key = EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, NULL, out->dec,
					strlen((const char *)out->dec));
	return out;
}

message *message_new2(int const size, const account *a) {
	message *out = malloc(sizeof(message));
	*out = (message){.data = malloc(BUF),
...
void generate_msg(const account *a1, message *m, const logon *login) {
	...
	sprintf((char *)m->tmp[0],
		...
		t->tm_min, t->tm_sec);
	sprintf(m->tmp[3], "%s\1%s\1%s\1%s\1Coinbase\1%s", m->tmp[0], m->tmp[1],
		m->tmp[2], login->api, login->pass);
	...
	sprintf(m->tmp[6],
		"49=%s|554=%s|96=%s|8013=S|52=%s|56=Coinbase|98=0|108=30|34=1|"
		"9406=N|",
		login->api, login->pass, m->tmp[5], m->tmp[0]);
	...
	for (size_t i = 0; i < strlen(m->tmp[7]); i++)
		(m->tmp[7][i] == BAR) ? m->tmp[7][i] = SOH : 0;
